...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # For each action -> create action node -> check pre-conditions from previous state 
        for action in self.all_actions:
            node_a = PgNode_a(action)
            if node_a.prenodes.issubset(self.s_levels[level]):
                # Add node to action level
                self.a_levels[level].add(node_a)
                # Connect action nodes and S nodes
    ...
    def add_literal_level(self, level):
        ...
        # For each action in previous level -> Add effect node to the new level -> take care of connection
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check node_a1's add effects are not in node_a2's remove effects
        for effect in node_a1.action.effect_add:
            if effect in node_a2.action.effect_rem:
                return True
        # Check node_a2's add effects are not in node_a1's remove effects
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Check node_a1's add/remove effects are not in node_a2's neg/pos precondition effects
        for effect in node_a1.action.effect_add:
        ...
        for effect in node_a1.action.effect_rem:
            if effect in node_a2.action.precond_pos:
                return True
       # Check node_a2's add/remove effects are not in node_a1's neg/pos precondition effects
        for effect in node_a2.action.effect_add:
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Loop through node_a1 and node_a2's pre-conditions -> Check is they are mutex
    ...
    def h_levelsum(self) -> int:
        ...
        # Loop through each goal -> check at which level the goal is met -> add the level number to levelsum
        for goal in self.problem.goal:
            goal_found = False
            for level in range(len(self.s_levels)):
                for state in self.s_levels[level]:
                    literal = expr(state.symbol)
                    if state.is_pos == False:
