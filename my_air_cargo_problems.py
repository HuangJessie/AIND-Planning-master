...
class AirCargoProblem(Problem):
    ...
    def get_actions(self):
        ...
        def unload_actions():
            ...
            # loop through all variables -> define precondition, effects -> excute action
            for airport in self.airports:
                for plane in self.planes:
                    for cargo in self.cargos:
    ...
    def actions(self, state: str) -> list:
        ...
        :return: list of Action objects
        """
        # Decode states to pos and neg
        possible_actions = []
        kb = PropKB()
        kb.tell(decode_state(state, self.state_map).pos_sentence())
        # For each action, check if stats of pos and neg are both valid -> add action to the list
        for action in self.actions_list:
            ...
            for precond_n in action.precond_neg:
                if precond_n not in kb.clauses:
                    is_valid = False
                    break            
            if is_valid == True:
    ...
    def result(self, state: str, action: Action):
        ...
        # For each fluent, check the effect in old state and update in new state
